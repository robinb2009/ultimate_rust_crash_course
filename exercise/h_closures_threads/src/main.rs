...
fn main() {
    ...
    println!("Main thread challenge part");
    
    let (tx, rx_a) = channel::unbounded();
    let rx_b = rx_a.clone();
    ...
    let handle_a = thread::spawn(move || loop {
        ...
        let res = rx_a.recv();
        match res {
            Ok(val) => println!("Thread A received value: {}", val),
            Err(err) => {
                println!("Thread A {}", err);
                return;
            },
        };
    });
    ...
    let handle_b = thread::spawn(move || loop {
        ...
        let res = rx_b.recv();
        match res {
            Ok(val) => println!("Thread B received value: {}", val),
            Err(err) => {
                println!("Thread B {}", err);
                return;
            },
        };
    });
